...
function LoginModal({isOpen, onClose}) {
  ...
  async function handleSignIn () {
    ...
    if (inpUsername === '') {setUsernameError('*Username is required'); return;}
    if (inpPassword === '') {setPasswordError('*Password is required'); return;}

    try {
      const response = await axios.post ('/api/users/login', {userName: inpUsername, userPassword: inpPassword});
        ...
        <div className='modal-content clickables basic-container'>
          ...
          <div style={{ marginBottom: '0.35rem', padding: '0 0.2rem'}}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <p>Username</p>
              { signInAttempted && <p style={{ color: 'red', fontSize: '0.72rem', alignSelf: 'flex-end'}}>{usernameError}</p>}
            </div>
          ...
          <div style={{ marginBottom: '0.35rem', padding: '0 0.2rem'}}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <p>Password</p>
              {signInAttempted && <p style={{ color: 'red', fontSize: '0.72rem', alignSelf: 'flex-end'}}>{passwordError}</p>}
            </div>
